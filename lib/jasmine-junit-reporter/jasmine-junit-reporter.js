/**
 * Adapted from Larry Myers' work on JKasmine Reporters
...
var JUnitXmlReporter = function(savePath, consolidate, useDotNotation) {
        savePath = savePath || '';
        jasmine.getGlobal().console.log("SAVE PATH IS " + this.savePath + " FROM " + savePath);
        ...
        reportRunnerResults: function(runner) {
            ...
            for (var i = 0; i < suites.length; i++) {
                ...
                if (this.consolidate && suite.parentSuite) {
                    ...
                    output += "\n</testsuites>";
                    this.writeFile(savePath + fileName, output);
                }
                else {
                    output += suite.output;
                    this.writeFile(savePath + fileName, output);
                ...
                allResults.passedCount += results.passedCount;
                allResults.failedCount += results.failedCount;
                allResults.totalCount += results.totalCount;
            }
            
                        EnvJasmine.passedCount = allResults.passedCount;
            EnvJasmine.failedCount = allResults.failedCount;
            EnvJasmine.totalCount = allResults.totalCount;
